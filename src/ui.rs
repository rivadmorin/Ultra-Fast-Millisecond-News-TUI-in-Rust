...
fn draw_main_view(f: &mut Frame, app: &mut App, area: Rect, colors: &ThemeColors) {
    ...
    let target_chunk = if app.view_mode == ViewMode::GlobalSearch {
        content_chunks[0]
    } else {
        content_chunks[1]
    };
...
fn draw_header(f: &mut Frame, app: &App, area: Rect, colors: &ThemeColors) {
    ...
    let mode_str = if app.view_mode == ViewMode::GlobalSearch {
        "GLOBAL SEARCH"
    } else if is_active {
        "ACTIVE"
    } else {
    ...
    let mode_color = if app.view_mode == ViewMode::GlobalSearch {
        Color::Magenta
    } else if is_active {
        colors.green
    } else {
...
fn draw_search_popup(f: &mut Frame, app: &App, colors: &ThemeColors) {
    ...
    let title = if app.view_mode == ViewMode::GlobalSearch {
        " Refine Global Search "
    } else {
        " Stealthy Search "
    };
...
fn draw_help_popup(f: &mut Frame, _app: &App, colors: &ThemeColors) {
    ...
    let text = vec![
        ...
        Line::from(vec![
            ...
            Span::raw(" Local Search (Current Category)"),
        ]),
        Line::from(vec![
            Span::styled(" [s] ", Style::default().fg(colors.green)),
            Span::raw(" Global Search (DuckDuckGo Engine)"),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw(" Back / Close popup / Exit Search"),
        ]),
        Line::from(vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "Pro Tip: Global search works in any mode via 's'.",
            Style::default().fg(colors.gray),
        )),
        Line::from(Span::styled(
            "Powered by Rust, SQLite, Scrapling & DDG",
            Style::default().fg(colors.gray),
        )),
    ];
...
fn draw_loading_popup(f: &mut Frame, colors: &ThemeColors) {
    ...
    let text = Paragraph::new(" Loading data from stealthy sources...")
        .block(block)
        .style(Style::default().fg(colors.text));
